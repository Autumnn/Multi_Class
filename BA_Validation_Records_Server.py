...
for i_test in range(4, 10):
    ...
    for Dir in DIRS:
        ...
        list_path = save_path + '/BA_Timeline_Record_Validation_' + str(i_test) + '/' + Dir
        os.makedirs(list_path)
        para_path = save_path + '/BA_Validation_' + str(i_test) + '/' + Dir
        ...
        for file in files:
            ...
            with open('BA_Opt_G_Mean_Validation_Records_' + str(i_test) + '.txt', 'a') as w:
                line = sub_name + '\t' + str(xgboostBO.res['max']['max_val']) + '\n'
                w.write(line)

            para_file = para_path + '/' + sub_name + '_BA.json'
            ...
            list_file = list_path + '/' + sub_name + '_BA_List.json'
            Output_Para = []
